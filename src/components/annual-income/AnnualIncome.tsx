...
// Equivalent to Angular AnnualIncomeComponent
...
export default function AnnualIncome() {
  ...
  return (
    <section aria-label="Annual Income Selection" className="pan_details_form">
          ...
          <div className="col-lg-10 col-12 m-auto">
            ...
            <div className="mobile_css">
              <div className="back_cls">
                {rejectStatus !== 'R' && (
                  <button type="button" className="figma-card-back" onClick={BackToTwo} aria-label="Go back">
                    <BackArrowIcon />
                  </button>
                )}
            ...
            <div className="figma-card">
              ...
              <div className="figma-card-body">
                <div className="figma-option-grid" role="group" aria-label="Annual Income options">
                  {incomeResponse.map((item: any, i: number) => (
                    <button
                      key={i}
                      type="button"
                      className={`figma-option-btn${selectedIncome === item.incomeRange ? ' selected' : ''}`}
                      onClick={() => PersonalDetailsave('annualincome', item.incomeRange)}
                      aria-pressed={selectedIncome === item.incomeRange}
                    >
                      {item.incomeRange}
                    </button>
                  ))}

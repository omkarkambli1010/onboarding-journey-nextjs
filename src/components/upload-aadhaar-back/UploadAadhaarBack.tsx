...
import styles from './upload-aadhaar-back.module.scss';

// UploadAadhaarBack — Upload back side of Aadhaar card
// Equivalent to Angular UploadAadhaarBackComponent

export default function UploadAadhaarBack() {
  ...
  const getAadhaarBackData = async () => {
    showSpinner();
    const reqData = {
      flag: 'AADHARBACK',
  ...
  const upload = async () => {
    if (!imageBase64 && !imagePreview) {
      toast.warning('Please upload your Aadhaar back image.', { position: 'bottom-center', autoClose: 2000 });
      return;
    }
    showSpinner();
    const reqData = {
    ...
    try {
      ...
      if (response?.status === true) {
        toast.success('Aadhaar back uploaded!', { position: 'bottom-center', autoClose: 2000 });
  ...
  const backToAadhaarFront = () => {
    showSpinner();
    setTimeout(() => { router.push('/aadhaar-front'); hideSpinner(); }, 200);
  };

  return (
    <section aria-label="Upload Aadhaar Back" className="pan_details_form">
          ...
          <div className="col-lg-10 col-12 m-auto">
            <div className="mobile_css">
              <div className="back_cls">
                {rejectStatus !== 'R' && (
                  <div onClick={backToAadhaarFront} style={{ cursor: 'pointer' }}>
                ...
                <div className="mobile_header_padding">
                  <div className="help_faq_css">
                    <div className="d-flex flex-column gap-2">
                      <h5>Upload Aadhaar (Back)</h5>
                    </div>
                    <div>
                      <div className="help_btn" onClick={() => faqHelpBtn('Aadhaar')} style={{ cursor: 'pointer' }}>Need Help?</div>
                    </div>
                  </div>
                  <p className="sub_title">Upload the back side of your Aadhaar card</p>
                </div>
              </div>
            </div>

            <form aria-label="Upload Aadhaar Back Form" method="post">
              <div>
                          ...
                          <div onClick={backToAadhaarFront} style={{ cursor: 'pointer' }}>
                            <span>
                              <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clipPath="url(#clip0_aadhar_back)">
                                ...
                                <defs><clipPath id="clip0_aadhar_back"><rect width="24" height="24" fill="white" transform="translate(0 0.5)" /></clipPath></defs>
                              </svg>
                            </span>
                          </div>
                        )}
                ...
                <div className="upload_section">
                  ...
                  >
                    {imagePreview ? (
                      <img src={imagePreview} alt="Aadhaar back preview" style={{ maxWidth: '100%', maxHeight: 300, borderRadius: 8 }} />
                    ) : (
                      <div>
                        <img src="/assets/images/diy/upload-icon.png" alt="Upload" />
                        <p>Click to upload Aadhaar back side</p>
